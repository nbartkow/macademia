<div class="medium padded2" id="coll_request">
  <div id="coll_top_container">
    ...
    <p id="coll_created"><span>Created:</span> on <g:formatDate format="MMMMMMMM d, yyyy" date="${collaboratorRequest?.dateCreated}"/> by <g:link url="#/?nodeId=p_${collaboratorRequest?.creator?.id}&navFunction=person&personId=${collaboratorRequest?.creator?.id}">${collaboratorRequest?.creator?.encodeAsHTML()}</g:link></p>
